...
function GetPlayerBySource(src) return QBCore.Functions.GetPlayer(src) end

function AddItem(src, item, amount)
    local Player = GetPlayerBySource(src)
    if not Player then return false end
    if Config.Inventory == 'ox' then return exports.ox_inventory:AddItem(src, item, amount or 1) end
...
function RemoveItem(src, item, amount)
    local Player = GetPlayerBySource(src)
    if not Player then return false end
    if Config.Inventory == 'ox' then return exports.ox_inventory:RemoveItem(src, item, amount or 1) end
