...
RegisterNetEvent('lol-weddings:server:buyRing', function(item)
    ...
    if not entry then TriggerClientEvent('QBCore:Notify', src, 'Invalid shop item.', 'error') return end
    if GetMoney(src, 'bank') < entry.price then TriggerClientEvent('QBCore:Notify', src, 'Not enough money in bank.', 'error') return end
