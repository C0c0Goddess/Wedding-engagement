RegisterCommand(Config.Commands.WeddingAdmin, function(source, args)
    local src = source
    if not QBCore.Functions.HasPermission(src, Config.Admin.Permission) then TriggerClientEvent('QBCore:Notify', src, 'No permission.', 'error') return end
    local action = args[1]
    if action == 'list' then
    ...
    if action == 'setstatus' then
        ...
        local status = args[3]
        if not id or not status then TriggerClientEvent('QBCore:Notify', src, 'Usage: /weddingadmin setstatus [id] [status]', 'error') return end
