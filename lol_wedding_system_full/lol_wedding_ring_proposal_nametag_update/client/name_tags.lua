    ...
    while true do
        ...
        if Config.MarriedNameTag.Enabled then
            ...
            for _, player in pairs(GetActivePlayers()) do
                nearby[#nearby + 1] = GetPlayerServerId(player)
            end
        ...
        if Config.MarriedNameTag.Enabled then
            ...
            for _, player in pairs(GetActivePlayers()) do
                ...
                if tag and DoesEntityExist(ped) then
                    ...
                    if dist <= Config.MarriedNameTag.DrawDistance then
                        DrawWeddingText3D(coords.x, coords.y, coords.z + Config.MarriedNameTag.ZOffset, tag)
                    end
                end
            end
        end
...
function DrawWeddingText3D(x, y, z, text)
