RegisterCommand(Config.Commands.Marry, function(source, args)
    ...
    if not targetId then
        TriggerClientEvent('QBCore:Notify', src, 'Usage: /marry [player id]', 'error')
        return
    end
    ...
    local sharedLastName = Player.PlayerData.charinfo.lastname
    ...
    local changed = MySQL.update.await([[
        ...
        SET status = 'married',
            ...
            married_at = NOW()
        WHERE
        ((citizenid1 = ? AND citizenid2 = ?) OR (citizenid1 = ? AND citizenid2 = ?))
        AND status = 'engaged'
        LIMIT 1
    ]], {
        sharedLastName,
...
RegisterCommand(Config.Commands.Divorce, function(source)
    ...
    if GetMoney(src, 'bank') < Config.Prices.Divorce then
        TriggerClientEvent('QBCore:Notify', src, 'Not enough money for divorce processing.', 'error')
        return
    end
    ...
    local changed = MySQL.update.await([[
        UPDATE marriages
        SET status = 'divorced'
        WHERE (citizenid1 = ? OR citizenid2 = ?)
        AND status = 'married'
        LIMIT 1
    ]], { cid, cid })
    ...
    if changed and changed > 0 then
        ...
        SendWeddingLog('Divorce Processed', cid .. ' processed a divorce.')
    else
...
RegisterCommand(Config.Commands.MarriageStatus, function(source)
    ...
    if not row then
        TriggerClientEvent('QBCore:Notify', src, 'No relationship record found.', 'primary')
        return
    end
...
RegisterCommand(Config.Commands.CreateLicense, function(source, args)
    ...
    if not targetId or not Target then
        TriggerClientEvent('QBCore:Notify', src, 'Usage: /marriagelicense [player id]', 'error')
        return
    end

    if GetMoney(src, 'bank') < Config.Prices.MarriageLicense then
        TriggerClientEvent('QBCore:Notify', src, 'Not enough money for marriage license.', 'error')
        return
    end
...
RegisterCommand(Config.Commands.NameChange, function(source, args)
    ...
    if not newLast then
        TriggerClientEvent('QBCore:Notify', src, 'Usage: /changemarriedname [lastname]', 'error')
        return
    end

    if GetMoney(src, 'bank') < Config.Prices.NameChange then
        TriggerClientEvent('QBCore:Notify', src, 'Not enough money for name change.', 'error')
        return
    end
...
RegisterNetEvent('lol-weddings:server:startCeremony', function(targetId)
    ...
    local pName = Player.PlayerData.charinfo.firstname
    local tName = Target.PlayerData.charinfo.firstname

    TriggerClientEvent('lol-weddings:client:playCeremony', src, tName)
    TriggerClientEvent('lol-weddings:client:playCeremony', targetId, pName)
end)
