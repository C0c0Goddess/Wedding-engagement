RegisterCommand(Config.Commands.SharedAccount, function(source)
    ...
    if not account then
        TriggerClientEvent('QBCore:Notify', src, 'No shared couple account found. Open it in the planner.', 'error')
        return
    end
...
RegisterNetEvent('lol-weddings:server:openSharedAccount', function()
    ...
    local marriage = MySQL.single.await([[
        SELECT *
        FROM marriages
        WHERE (citizenid1 = ? OR citizenid2 = ?)
        AND status = 'married'
    ...
    if not marriage then
        TriggerClientEvent('QBCore:Notify', src, 'You must be married to open a shared account.', 'error')
        return
    end
    ...
    if exists then
        TriggerClientEvent('QBCore:Notify', src, 'Shared account already exists.', 'error')
        return
    end

    if GetMoney(src, 'bank') < Config.Prices.SharedAccountOpen then
        TriggerClientEvent('QBCore:Notify', src, 'Not enough money to open shared account.', 'error')
        return
    end
    ...
    MySQL.insert.await([[
        ...
        (marriage_id, citizenid1, citizenid2, balance, created_at)
        VALUES (?, ?, ?, 0, NOW())
    ]], {
        marriage.id,
        marriage.citizenid1,
...
RegisterNetEvent('lol-weddings:server:depositShared', function(amount)
    ...
    if GetMoney(src, 'bank') < amount then
    ...
    if not account then
        TriggerClientEvent('QBCore:Notify', src, 'No shared account found.', 'error')
        return
    end
...
RegisterNetEvent('lol-weddings:server:withdrawShared', function(amount)
    ...
    if not account or account.balance < amount then
        TriggerClientEvent('QBCore:Notify', src, 'Insufficient shared funds.', 'error')
        return
    end
