RegisterNetEvent('lol-weddings:server:getPlannerData', function()
    ...
    TriggerClientEvent('lol-weddings:client:openPlanner', src, {
...
RegisterNetEvent('lol-weddings:server:createWeddingPlan', function(data)
    ...
    MySQL.insert.await([[
        ...
        (owner, venue, budget, guests, wedding_date, theme, created_at)
        VALUES (?, ?, ?, ?, ?, ?, NOW())
    ]], {
        Player.PlayerData.citizenid,
        data.venue or 'undecided',
        tonumber(data.budget) or 0,
        json.encode(data.guests or {}),
        data.weddingDate or 'TBD',
        data.theme or 'Luxury Gold'
    })
...
RegisterNetEvent('lol-weddings:server:bookVenue', function(venueId)
    ...
    for _, venue in pairs(Config.Venues) do
        if venue.id == venueId then selected = venue end
    end

    if not selected then
        TriggerClientEvent('QBCore:Notify', src, 'Invalid venue.', 'error')
        return
    end

    if GetMoney(src, 'bank') < selected.price then
        TriggerClientEvent('QBCore:Notify', src, 'Not enough money to book this venue.', 'error')
        return
    end
    ...
    MySQL.insert.await([[
        ...
        (owner, venue_id, venue_name, price, status, created_at)
        VALUES (?, ?, ?, ?, 'booked', NOW())
    ]], {
        Player.PlayerData.citizenid,
        selected.id,
...
RegisterNetEvent('lol-weddings:server:inviteGuest', function(targetId)
    ...
    if not Player or not Target then
        TriggerClientEvent('QBCore:Notify', src, 'Guest not found.', 'error')
        return
    end

    MySQL.insert.await([[
        INSERT INTO wedding_guests
        (owner, guest_citizenid, guest_name, rsvp, created_at)
        VALUES (?, ?, ?, 'pending', NOW())
    ]], {
        Player.PlayerData.citizenid,
